...
| **Dark Web / Darknet (P8)** | `GET /api/darkweb?q=...` — Ahmia/DarkSearch by default; optional Tor engines; `GET /api/darkweb/engines`, `GET /api/darkweb/mentions`, `GET /api/darkweb/entities?q=...`; `POST /api/darkweb/ingest`, `POST /api/darkweb/scrape`, `POST /api/darkweb/deep_search`; ransomware leak-site intel via `GET /api/darkweb/ransomware/groups`, `GET /api/darkweb/ransomware/victims`, `POST /api/darkweb/ransomware/refresh`, `POST /api/darkweb/ransomware/ingest`; ransomware briefing block (max 5 lines, FTM-correlation prioritised) when `WORLDBASE_BRIEFING_RANSOMWARE=1`; breach/credential-leak intel via `GET /api/darkweb/breach/status`, `POST /api/darkweb/breach/check`, `POST /api/darkweb/breach/password`, `POST /api/darkweb/breach/monitor`, `GET /api/darkweb/breach/monitors`, `DELETE /api/darkweb/breach/monitor/{id}`, `POST /api/darkweb/breach/refresh`; HIBP API v3 + Pwned Passwords k-anonymity; SQLite monitor table (SHA1 hash + base64 email); briefing BREACH block when `WORLDBASE_BRIEFING_BREACH=1`; `WORLDBASE_BREACH=1`, `WORLDBASE_HIBP_API_KEY` (required for email checks, password checks work without key); DATA → **DARKWEB** tab → **BREACH** sub-tab; `WORLDBASE_DARKWEB=1`, `WORLDBASE_BRIEFING_DARKWEB=1`, `WORLDBASE_RANSOMWARE=1`; docs → [`docs/DARKWEB.md`](docs/DARKWEB.md) |
