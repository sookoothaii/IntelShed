...
| **Dark Web / Darknet (P8)** | `GET /api/darkweb?q=...` — Ahmia/DarkSearch by default; optional Tor engines; `GET /api/darkweb/engines`, `GET /api/darkweb/mentions`, `GET /api/darkweb/entities?q=...`; `POST /api/darkweb/ingest`, `POST /api/darkweb/scrape`, `POST /api/darkweb/deep_search`; ransomware leak-site intel via `GET /api/darkweb/ransomware/groups`, `GET /api/darkweb/ransomware/victims`, `POST /api/darkweb/ransomware/refresh`, `POST /api/darkweb/ransomware/ingest`; ransomware briefing block (max 5 lines, FTM-correlation prioritised) when `WORLDBASE_BRIEFING_RANSOMWARE=1`; breach/credential-leak intel via `GET /api/darkweb/breach/status` (shows `provider`: "hibp" or "xposedornot"), `POST /api/darkweb/breach/check`, `POST /api/darkweb/breach/password`, `POST /api/darkweb/breach/monitor`, `GET /api/darkweb/breach/monitors`, `DELETE /api/darkweb/breach/monitor/{id}`, `POST /api/darkweb/breach/refresh`; HIBP API v3 (with key) + XposedOrNot free API fallback (no key needed, 100 req/day) for email checks; Pwned Passwords k-anonymity for password checks (no key needed); SQLite monitor table (SHA1 hash + base64 email); briefing BREACH block when `WORLDBASE_BRIEFING_BREACH=1`; MCP tools: `worldbase_breach_status`, `worldbase_breach_check_password`; `WORLDBASE_BREACH=1`, `WORLDBASE_HIBP_API_KEY` (optional — email checks fall back to XposedOrNot without key; password checks always work); DATA → **DARKWEB** tab → **BREACH** sub-tab; `WORLDBASE_DARKWEB=1`, `WORLDBASE_BRIEFING_DARKWEB=1`, `WORLDBASE_RANSOMWARE=1`; V4-58 engine-specific HTML parsers (BeautifulSoup4) for Torch, Tor66, TorDex, Haystak, Not Evil — replaces generic `_parse_tor_html()` heuristic with dedicated selectors + fallback link extraction (`backend/darkweb_parsers.py`); docs → [`docs/DARKWEB.md`](docs/DARKWEB.md) |
...
| **MCP (Cursor)** | Streamable HTTP `http://127.0.0.1:8002/api/mcp` — 15 tools when Agent Bus on — [`docs/MCP.md`](docs/MCP.md); `worldbase_chat` default provider: `nvidia` (`stepfun-ai/step-3.7-flash`), override via `WORLDBASE_MCP_MODEL` / `WORLDBASE_MCP_PROVIDER` env vars; breach tools: `worldbase_breach_status`, `worldbase_breach_check_password` |
