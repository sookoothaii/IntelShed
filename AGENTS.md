...
| **Maritime Anomaly Detection (P7)** | `GET /api/maritime/anomalies`, `GET /api/maritime/trajectory/{mmsi}`, `GET /api/maritime/trajectory/stats` — AIS trajectory storage (`data/ais_trajectory.db`), behavioural anomaly detection (speed variance, AIS gaps >2h, night port visits, course changes >15°, risk zone proximity); in-memory ringbuffer + batch flush; `WORLDBASE_MARITIME_TRAJECTORY=1` (default off), `WORLDBASE_MARITIME_ANOMALY_THRESHOLD=0.6`; briefing bridge (`backend/maritime_briefing.py`) → MARITIME ANOMALIES block in prompt + watch items + FtM Vessel correlation by MMSI; lifespan background task flushes ringbuffer + prunes old positions every 5 min; `backend/ais_trajectory.py` |
