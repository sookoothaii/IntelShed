...
| **Document Export** | `GET /api/briefing/export?format=pdf` / `?format=docx` — downloadable briefing report (reportlab + python-docx, 0 VRAM); `GET /api/connectors/export?format=xlsx` — feed status spreadsheet (openpyxl); FULL SITUATION → PDF / DOCX buttons next to GENERATE — `backend/doc_export.py` |
...
| Chat + LLM proxy | `backend/routes/chat.py` (compat), `backend/chat_context.py` (context builder + search), `backend/chat_proxy.py` (models/chat/providers) — 6 providers: ollama, openai, anthropic, groq, openrouter, **nvidia** (NIM, OpenAI-compatible); `NVIDIA_API_KEY`, `NVIDIA_BASE_URL` (default `https://integrate.api.nvidia.com/v1`); models: `deepseek-ai/deepseek-v4-flash`, `qwen/qwen3.5-122b-a10b`, `qwen/qwen3.5-397b-a17b`; `WORLDBASE_CHAT_PROVIDER=nvidia` + `WORLDBASE_CHAT_MODEL=deepseek-ai/deepseek-v4-flash`. **Chat tools:** `backend/chat_tools.py` — `focus_globe` (place geocoding via OpenStreetMap Nominatim; ignores LLM-guessed lat/lon when `place` provided), `geocode_place` (standalone geocoding), `spatial_query`, `entity_context`; MCP `worldbase_globe_fly_to` mirrors `focus_globe` with geocoding. **NVIDIA stream fix:** empty `choices: []` chunks handled via `or [{}]` pattern (was `IndexError: list index out of range`). |
