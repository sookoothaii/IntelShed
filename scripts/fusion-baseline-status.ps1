...
if ($Json) {
    ...
    Write-Output "  snapshots_stored=$stored / $targetSnapshots ($pct%)"
    Write-Output "  fusion_compare.available=$available"
    if ($fc.detail) { Write-Output "  detail=$($fc.detail)" }
    if ($available -and $fc.top_delta) {
        Write-Output "  top_delta=$($fc.top_delta.cell_id) delta=$($fc.top_delta.delta_score)"
    }
}
