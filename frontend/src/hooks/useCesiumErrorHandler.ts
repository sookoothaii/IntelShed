...
type GlobeWithTileError = Globe & {
  tileLoadErrorEvent?: {
    addEventListener(fn: (e: CesiumTileLoadErrorEvent) => void): void;
    removeEventListener(fn: (e: CesiumTileLoadErrorEvent) => void): void;
  };
};
    ...
    return () => {
      ...
      try {
        if (tileLoadErrorListenerRef.current && !v.isDestroyed?.()) {
          (scene.globe as GlobeWithTileError).tileLoadErrorEvent?.removeEventListener(
            tileLoadErrorListenerRef.current,
          );
        }
      } catch {
        /* torn down */
