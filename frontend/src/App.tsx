...
import { agenticBadgeMeta } from './lib/agenticTrace'
import { useSituationsQuery, useBriefingQuery, useHealthPingQuery, useModelsQuery } from './hooks/useHudQueries'
...
function useSituationsBadge() {
  ...
  return {
    ...
    tone: (high > 0 ? 'warn' : 'ok') as 'ok' | 'warn',
  }
}
...
function SystemStatus() {
  const { data: ping, isLoading: pingLoading } = useHealthPingQuery()
  const { data: models, isLoading: modelsLoading } = useModelsQuery()
  const backend = pingLoading ? 'check' : (ping?.ok ? 'online' : 'offline')
  const ollama = modelsLoading ? 'check' : (models?.online ? 'online' : 'offline')
