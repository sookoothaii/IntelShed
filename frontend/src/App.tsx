...
function TabFallback({ label = 'Loading' }: { label?: string }) {
  return (
    ...
    >
      <div className="boot-line" style={{ opacity: 0.5, animationDelay: '0s' }}>
        {label}…
      </div>
    </div>
  );
      ...
      try {
        ...
        const titles: string[] = [
          ...
          ...anomalies
            .slice(0, 3)
            .map(
              (a: { callsign?: string; icao24?: string }) => `Anomaly ${a.callsign || a.icao24}`,
            ),
        ];
...
function useSituationsBadge() {
  ...
  const high = items.filter((i) => i.severity === 'critical' || i.severity === 'high').length;
  return {
    label: String(count),
...
function SystemStatus() {
  ...
  const backend: 'online' | 'offline' | 'check' = ping.isLoading
    ? 'check'
    : ping.isError
      ? 'offline'
      : 'online';
  const ollama: 'online' | 'offline' | 'check' = models.isLoading
    ? 'check'
    : models.isError || models.data?.error
      ? 'offline'
      : 'online';

  return (
    <div className="sys-status">
      <span className={`sys-pip ${backend}`} />
      BACKEND
      <span className={`sys-pip ${ollama}`} />
      OLLAMA
    </div>
  );
}
...
export default function App() {
  ...
  const [askAI, setAskAI] = useState<{ id: number; question: string; context: string } | null>(
    null,
  );
  ...
  const [mapMode, setMapMode] = useHudSessionState<MapViewMode>(
    'mapMode',
    DEFAULT_MAP_VIEW,
    isMapViewMode,
  );
  ...
  const [layoutMode, setLayoutMode] = useHudSessionState<LayoutMode>(
    'layoutMode',
    'full',
    (v): v is LayoutMode => v === 'full' || v === '3col',
  );
  const [railTab, setRailTab] = useState<RailTab>('overview');
  const [leftCollapsed, setLeftCollapsed] = useHudSessionState(
    'leftCollapsed',
    false,
    (v): v is boolean => v === true || v === false,
  );
  const [rightCollapsed, setRightCollapsed] = useHudSessionState(
    'rightCollapsed',
    false,
    (v): v is boolean => v === true || v === false,
  );
  ...
  const handleGlobeMove = useCallback(
    (cam: { lon: number; lat: number; height: number; pitch?: number }) => {
  ...
  const handleAskAI = (title: string, lines: string[]) => {
    ...
    const question =
      'Analyze this target and tell me what it means for the current world situation.';
    ...
    <div className="app">
      ...
      {calTrigOpen && (
        <Suspense fallback={<TabFallback label="CAL & TRIG" />}>
          <ErrorBoundary name="CalibrationTriggers">
            <CalibrationTriggersPanel onClose={() => setCalTrigOpen(false)} />
          </ErrorBoundary>
        </Suspense>
      )}
      {kanbanOpen && briefingKanbanEnabled && (
        <Suspense fallback={<TabFallback label="PIPELINE" />}>
          <ErrorBoundary name="BriefingKanban">
            <BriefingKanban onClose={() => setKanbanOpen(false)} />
          </ErrorBoundary>
        </Suspense>
      )}
      {analysisOpen && (
            ...
            <div key={view} className="view-fade">
              {view === 'data' && (
                <Suspense fallback={<TabFallback label="DATA" />}>
                  <DataPanel
                    onFocus={focusOnMap}
                    onOpenWindyMap={openWindyMap}
                    intelEntityId={intelEntityId}
                  />
                </Suspense>
              )}
              {view === 'news' && (
        ...
        {layoutMode === '3col' && (
          <Suspense fallback={<TabFallback label="BRIEFING" />}>
            <SidebarRight
              ...
              onFocus={(lat, lon, title) =>
                focusOnMap({ kind: 'sidebar', lat, lon, height: 400000, title, lines: [] })
              }
