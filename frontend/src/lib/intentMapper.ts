...
const LAYER_RULES: LayerRule[] = [
  { keywords: ['quake', 'seismic', 'earthquake', 'tremor'], layer: 'quakes', label: 'seismic events' },
  ...
  { keywords: ['storm', 'cyclone', 'hurricane', 'typhoon', 'gdacs', 'disaster'], layer: 'gdacs', label: 'disaster alerts' },
  { keywords: ['hazard', 'warning', 'cap', 'weather alert'], layer: 'hazards', label: 'weather warnings' },
  { keywords: ['ship', 'vessel', 'ais', 'maritime', 'boat', 'cargo', 'tanker'], layer: 'maritime', label: 'maritime traffic' },
  { keywords: ['aircraft', 'plane', 'flight', 'aviation', 'adsb'], layer: 'aircraft', label: 'aircraft tracking' },
  { keywords: ['military', 'war', 'conflict', 'armed'], layer: 'military', label: 'military assets' },
  { keywords: ['satellite', 'orbit', 'leo', 'gps', 'starlink'], layer: 'satellites', label: 'satellites' },
  { keywords: ['outage', 'internet', 'disconnect', 'blackout'], layer: 'outages', label: 'internet outages' },
  { keywords: ['geopolitic', 'crisis', 'tension', 'sanction'], layer: 'geopolitics', label: 'geopolitical crises' },
  { keywords: ['osint', 'intel', 'entity', 'ftm'], layer: 'osint', label: 'OSINT entities' },
  { keywords: ['darkweb', 'darknet', 'onion', 'tor'], layer: 'darkweb', label: 'darkweb intelligence' },
  { keywords: ['detection', 'box', 'anomaly', 'fusion'], layer: 'detectionBoxes', label: 'detection boxes' },
  { keywords: ['weather', 'temperature', 'wind', 'precipitation'], layer: 'weather', label: 'weather data' },
  { keywords: ['air quality', 'pollution', 'aqi', 'pm25', 'smog', 'haze'], layer: 'airquality', label: 'air quality' },
  { keywords: ['energy', 'power', 'electricity', 'grid'], layer: 'energy', label: 'energy data' },
  { keywords: ['flood', 'river', 'gauge', 'pegel', 'water level'], layer: 'pegel', label: 'river gauges' },
  { keywords: ['node', 'pi', 'edge', 'raspberry'], layer: 'nodes', label: 'edge nodes' },
  { keywords: ['transit', 'bus', 'train', 'metro', 'public transport'], layer: 'transit', label: 'transit' },
  { keywords: ['traffic cam', 'webcam', 'camera'], layer: 'trafficCams', label: 'traffic cameras' },
  { keywords: ['space weather', 'solar', 'kp index', 'geomagnetic', 'aurora'], layer: 'spaceweather', label: 'space weather' },
  { keywords: ['intel', 'ftm', 'follow the money', 'graph'], layer: 'intelFt', label: 'intel graph' },
  { keywords: ['satellite change', 'change detection', 'diff'], layer: 'satelliteChange', label: 'satellite change detection' },
...
const VISION_RULES: { keywords: string[]; mode: string; label: string }[] = [
  ...
  { keywords: ['normal', 'optical', 'default vision', 'clear'], mode: 'normal', label: 'normal optics' },
];

const HEATMAP_KEYWORDS = ['heatmap', 'heat map', 'fusion', 'hotspot', 'hot spot'];

const TOGGLE_OFF = ['hide', 'disable', 'turn off', 'deactivate', 'remove'];
const FLY_KEYWORDS = ['fly to', 'go to', 'zoom to', 'focus on', 'show me', 'navigate to', 'look at', 'center on'];
...
const BUILTIN_PLACES: Record<string, { lat: number; lon: number; height: number }> = {
  ...
  'iran': { lat: 32.4279, lon: 53.6880, height: 800000 },
  'tehran': { lat: 35.6892, lon: 51.3890, height: 100000 },
  ...
  'taipei': { lat: 25.0330, lon: 121.5654, height: 50000 },
  ...
  'myanmar': { lat: 21.9139, lon: 95.9560, height: 500000 },
  ...
  'seoul': { lat: 37.5665, lon: 126.9780, height: 50000 },
  ...
  'sanaa': { lat: 15.3694, lon: 44.1910, height: 40000 },
...
function matchPlace(query: string): { lat: number; lon: number; height: number; name: string } | null {
...
export function mapIntent(query: string): IntentResult {
  ...
  if (actions.length === 0 && !matched.some((m) => m.startsWith('geocode:'))) {
    return {
      actions,
      explanation: 'No globe actions matched. Try: "show me earthquakes near Thailand", "fly to Tehran", "enable thermal vision", "show wildfires and maritime".',
      matched,
    };
  }
  ...
  if (flyActions.length) parts.push(`Flying to ${(flyActions[0] as Extract<GlobeAction, { type: 'fly_to' }>).title}`);
  ...
  if (visionActions.length) parts.push(`Switching to ${(visionActions[0] as Extract<GlobeAction, { type: 'set_vision' }>).mode} vision`);
  if (heatmapActions.length) parts.push(`${(heatmapActions[0] as Extract<GlobeAction, { type: 'toggle_heatmap' }>).enabled ? 'Enabling' : 'Disabling'} fusion heatmap`);
...
export async function geocodePlace(place: string): Promise<{ lat: number; lon: number; display_name: string } | null> {
  try {
    const r = await fetch(
