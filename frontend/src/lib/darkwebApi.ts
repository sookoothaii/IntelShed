...
export async function ingestRansomwareVictims(
  ...
  const params = new URLSearchParams({ limit: String(limit) })
  if (group) params.set('group', group)
  const r = await fetchApi(`/api/darkweb/ransomware/ingest?${params.toString()}`, { method: 'POST' })
  if (!r.ok) throw new Error(`${r.status} ${r.statusText}`)
  return r.json()
}

export async function scrapeDarkwebUrl(url: string, extract = true): Promise<{
  url: string
  ok: boolean
  error?: string
  text: string
