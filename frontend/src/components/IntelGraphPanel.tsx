...
export default function IntelGraphPanel({ onFocus, initialEntityId }: Props) {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      setError(`status: ${(e as Error).message || e}`);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    fetchFeedStatus();
  }, [fetchStatus, fetchResolutionStatus, fetchFeedStatus]);

  // Keep wheel zoom on the graph only — do not scroll the DATA panel underneath.
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
    const stopPanelScroll = (e: WheelEvent) => {
      e.stopPropagation();
    };
  ...
  useEffect(() => {
    ...
    cy.on('tap', 'node', (evt) => {
      ...
      if (onFocus && typeof lat === 'number' && typeof lon === 'number') {
        onFocus({
          ...
          lines: [`SCHEMA: ${evt.target.data('schema')}`],
        });
      }
      setRootId(id);
      loadGraph(id);
    ...
    cy.on('mouseover', 'edge', (evt) => {
      ...
      setEdgeTip(
        `${d.label} · ${fmtConfidence(d.confidence)} · ${d.dataset || '—'}${d.seen_at ? ` · ${d.seen_at.slice(0, 19)}` : ''}`,
      );
    });
  ...
  const loadOverview = useCallback(async () => {
    ...
    try {
      ...
      setError(`overview: ${(e as Error).message || e}`);
    }
  ...
  const runFeeds = async () => {
    setBusy(true);
    setError(null);
    setInfo('Syncing live feeds into FtM graph…');
    try {
      const r = await fetchApi('/api/intel/feeds/run', { method: 'POST' });
      const d = await r.json();
      if (!r.ok) {
        setError(d.detail || `feed sync failed (${r.status})`);
        return;
      }
      const t = d.totals || {};
      ...
      const resLine = res
        ...
        : '';
      setInfo(
        `✓ feeds +${t.entities ?? 0} entities · ${t.records ?? 0} records${resLine} — loading overview…`,
      );
      fetchFeedStatus();
      await loadOverview();
    } catch (e: unknown) {
      setError(`feeds: ${(e as Error).message || e}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const runResolution = async () => {
    setBusy(true);
    setError(null);
    setInfo('Running entity resolution (Splink)…');
    try {
      const r = await fetchApi('/api/intel/resolution/run', { method: 'POST' });
      const d = await r.json();
      if (!r.ok) {
        setError(d.detail || `resolution failed (${r.status})`);
        return;
      }
      setInfo(
        `✓ resolution +${d.edges_added ?? 0} edges (${d.exact_edges ?? 0} exact · ${d.subset_edges ?? 0} subset · ${d.splink_edges ?? 0} splink)`,
      );
      fetchResolutionStatus();
      if (rootId) await loadGraph(rootId);
      else await loadOverview();
    } catch (e: unknown) {
      setError(`resolution: ${(e as Error).message || e}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const ingest = async () => {
    ...
    setBusy(true);
    setError(null);
    setInfo('Extracting entities + relations (first run loads the model)…');
    try {
      ...
      if (!r.ok || !d.ok) {
        setError(d.error || `ingest failed (${r.status})`);
        return;
      }
      setInfo(
        `✓ ${d.counts?.entities ?? 0} entities · ${d.counts?.relations ?? 0} relations · ${d.counts?.mentions ?? 0} mentions · ${d.relations_mode ?? '?'}`,
      );
      fetchStatus();
      if (d.root_id) {
        setRootId(d.root_id);
      ...
      setError(`ingest: ${(e as Error).message || e}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      setInfo(
        `✓ ${d.counts?.entities ?? 0} entities · ${d.counts?.relations ?? 0} relations · ${d.device}`,
      );
      fetchStatus();
      if (d.root_id) {
        setRootId(d.root_id);
      ...
      setError(`upload: ${(e as Error).message || e}`);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="intel-panel">
          ...
          <>
            <span className={`stat-pill ${status.cuda_available ? 'ok' : 'warn'}`}>
              {status.cuda_available
                ? `GPU ${status.cuda_device?.replace('NVIDIA GeForce ', '') || 'CUDA'}`
                : 'CPU'}
            </span>
            ...
            <span className="stat-meta">
              {status.loaded
                ? `model loaded · ${status.device}`
                : 'model lazy (loads on first ingest)'}
            </span>
            ...
            {feedStatus && (
              <span className="stat-meta">
                ftm: {feedStatus.ftm_stats?.entities ?? 0} ent · {feedStatus.ftm_stats?.edges ?? 0}{' '}
                edges
      ...
      <div className="intel-section">
        ...
        <div className="intel-toolbar">
          <input
            ...
            title="Provenance dataset tag"
          />
          <button className="data-refresh" onClick={ingest} disabled={busy}>
            {busy ? '…' : 'INGEST'}
          </button>
      ...
      <div className="intel-section intel-section-graph">
        <h3>
          🕸 Graph <span className="stat-meta">{info || '—'}</span>
        </h3>
        <div className="intel-toolbar">
          <input
          ...
          <button
            ...
            disabled={busy}
            title="Show recent feed + ingest entities"
          >
            OVERVIEW
          </button>
          <button
            ...
            onClick={runFeeds}
            disabled={busy}
            title="Pull GDACS/GDELT/EONET/AIS/anomalies into FtM"
          >
            SYNC FEEDS
          </button>
          <button
            ...
            disabled={busy || !resStatus?.available}
            title="Splink entity resolution -> sameAs edges"
          >
            RESOLVE
          </button>
        </div>
        <div
          className="intel-schema-filter"
          title="Filter overview graph by FtM schema (excludes Airplane by default)"
        >
          {OVERVIEW_SCHEMAS.map((s) => (
            <button
              ...
              style={
                schemaFilter.has(s)
                  ? { borderColor: schemaColor(s), color: schemaColor(s) }
                  : undefined
              }
        ...
        <div className="intel-legend">
          ...
          <span className="intel-legend-hint">
            click node · hover edge for provenance · dashed = mentions · blue/green = sameAs
          </span>
        </div>
      </div>
    </div>
