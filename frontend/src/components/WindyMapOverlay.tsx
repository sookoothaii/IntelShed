...
type WindyApi = {
  ...
  picker?: {
    ...
    on: (event: string, cb: (data: { lat: number; lon: number; values: unknown; overlay: string }) => void) => void
  }
}
  ...
  return (
    <div className="windy-map-overlay" role="dialog" aria-label="Windy weather map">
      <div className="windy-map-toolbar">
        <span className="windy-map-title">WINDY MAP · {lat.toFixed(2)}, {lon.toFixed(2)}</span>
      ...
      {status === 'ready' && (
        <p className="windy-layer-hint">
          {mapTestingTier ? (
            <>
              <strong>Map Testing key</strong> — only WIND, TEMP, PRESSURE on this map. Rain, clouds, and
              STORMS (CAPE) need{' '}
              <a href="https://api.windy.com/map-forecast/pricing" target="_blank" rel="noopener noreferrer">
                Map Forecast Professional
              </a>
              . For rain now: enable globe layer <strong>WEATHER</strong> and use DATA → WEATHER (Point
              Forecast). Bottom slider = forecast hour.
            </>
          ) : (
            <>
              <strong>STORMS</strong> = CAPE (convection energy, not a thunder radar). Windy has no
              separate thunder layer — use <strong>RAIN</strong> or <strong>STORMS</strong>. Bottom slider
              = forecast hour.
            </>
          )}
        </p>
      ...
      {status === 'loading' && <div className="health-status pending windy-map-status">LOADING WINDY MAP…</div>}
      {status === 'error' && (
        <div className="health-status pending windy-map-status">{error || 'Windy map failed to load'}</div>
      )}
