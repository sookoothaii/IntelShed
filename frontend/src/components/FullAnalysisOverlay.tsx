...
import { useBriefingQuery } from '../hooks/useHudQueries'
import { hudQueryKeys } from '../hooks/useHudQueries'
import type { AgenticTrace } from '../lib/agenticTrace'
...
export default function FullAnalysisOverlay({ onClose, onFocus }: { onClose: () => void; onFocus: (f: Omit<FocusTarget, 'ts'>) => void }) {
  ...
  const { data: briefing, isLoading: briefingLoading, refetch: refetchBriefing } = useBriefingQuery()

  const analysisQueries = useQueries({
    queries: ANALYSIS_ENDPOINTS.map((ep) => ({
      queryKey: hudQueryKeys.analysis(ep.key),
      queryFn: async () => {
        try {
          ...
          return { error: e?.name === 'AbortError' ? 'timeout' : 'unavailable' }
        }
      },
      staleTime: 30_000,
      refetchInterval: (autoRefresh ? 30_000 : false) as false | number,
    })),
  })

  const results = Object.fromEntries(
    ANALYSIS_ENDPOINTS.map((ep, i) => [ep.key, analysisQueries[i]?.data]),
  ) as Record<string, any>

  const loading = briefingLoading || analysisQueries.some((q) => q.isLoading && q.data === undefined)
  ...
  const generateBriefing = async () => {
    ...
    try {
      ...
      await Promise.all([
        refetchBriefing(),
        queryClient.invalidateQueries({ queryKey: ['hud', 'analysis'] }),
      ])
    } catch (e: any) {
