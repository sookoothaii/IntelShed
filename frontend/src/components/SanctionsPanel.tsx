...
export default function SanctionsPanel({ onFocus }: Props) {
  ...
  const search = async () => {
    ...
    try {
      ...
      setError(`search: ${e instanceof Error ? e.message : String(e)}`);
      setResults([]);
    } finally {
      setSearching(false);
    }
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setError(`refresh: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
  ...
  const screen = useCallback(async () => {
    ...
    try {
      ...
      setError(`screen: ${e instanceof Error ? e.message : String(e)}`);
      setScreenHits([]);
    } finally {
  ...
  return (
    <div className="sanctions-panel">
      <div className="sanctions-status">
        {status ? (
          <>
            <span className={`stat-pill ${status.csv.fresh ? 'ok' : 'warn'}`}>
              BACKEND {status.backend.toUpperCase()}
            </span>
            <span className="stat-meta">
              {status.index_rows.toLocaleString()} entities · CSV {status.csv.size_mb} MB
            </span>
            <span className="stat-meta">
              {status.csv.age_sec != null
                ? `${(status.csv.age_sec / 3600).toFixed(1)}h old`
                : 'not loaded'}
            </span>
            <button className="data-refresh" onClick={refresh} disabled={refreshing}>
              {refreshing ? '…' : '↻ REFRESH CSV'}
            </button>
            <span className="stat-meta" title={status.license}>
              data: CC-BY · OpenSanctions
            </span>
          </>
        ) : (
      ...
      <div className="sanctions-section">
        ...
        <div className="sanctions-toolbar">
          ...
          <button className="data-refresh" onClick={search} disabled={searching}>
            {searching ? '…' : 'SEARCH'}
          </button>
        </div>
        {results.length > 0 && (
          <div className="sanctions-results">
            {results.map((r) => (
              <div
                key={r.entity_id}
                className={`sanc-card score-${Math.round((r.score ?? 0) * 10)}`}
              >
                ...
                <div className="sanc-meta">
                  ...
                  {(r.datasets?.length ?? 0) > 0 && (
                    <span>📚 {r.datasets!.slice(0, 4).join(', ')}</span>
                  )}
                </div>
                {(r.identifiers?.length ?? 0) > 0 && (
                  <div className="sanc-ids">
                    IDs: <code>{r.identifiers!.slice(0, 5).join(' · ')}</code>
                  </div>
                )}
                ...
                {r.url && (
                  <a href={r.url} target="_blank" rel="noreferrer" className="sanc-link">
                    OpenSanctions page ↗
                  </a>
                )}
              </div>
      ...
      <div className="sanctions-section">
        <h3>
          🚢 AIS ↔ Sanctions screen{' '}
          <button className="data-refresh small" onClick={screen} disabled={screenLoading}>
            {screenLoading ? '…' : '↻'}
          </button>
        </h3>
        {screenHits.length === 0 && !screenLoading && (
          <div className="data-info">
            No live vessel intersects the sanctions watchlist right now.
          </div>
        )}
        {screenHits.map((h, i) => (
          <div key={i} className="sanc-hit-row">
            ...
            <div className="hit-body">
              <div>
                <strong>{h.vessel.name || '?'}</strong>{' '}
                <span className="hit-mmsi">{h.vessel.mmsi || '—'}</span> · {h.vessel.type || '—'}
              </div>
            ...
            <button
              ...
              onClick={() => focusVessel(h)}
              disabled={h.vessel.lat == null}
            >
              ◎
            </button>
          </div>
