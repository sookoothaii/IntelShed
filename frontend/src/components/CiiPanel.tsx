...
function FamilyBar({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, marginBottom: 2 }}>
      <span style={{ width: 70, color: '#8fa8a0', textTransform: 'uppercase', fontSize: 10 }}>
        {label}
      </span>
      <div style={{ flex: 1, height: 4, background: '#222', borderRadius: 2 }}>
        <div
          style={{
            width: `${pct}%`,
            height: '100%',
            background: `hsl(${(1 - pct / 100) * 120}, 70%, 45%)`,
            borderRadius: 2,
          }}
        />
      </div>
      <span
        style={{ width: 28, textAlign: 'right', color: '#ccc', fontVariantNumeric: 'tabular-nums' }}
      >
        {value.toFixed(0)}
      </span>
    </div>
...
function CountryRow({ c }: { c: CiiCountry }) {
  ...
  const deltaColor =
    delta != null ? (delta > 0 ? '#ff6b35' : delta < 0 ? '#00e5a0' : '#888') : '#888';
  ...
  return (
    ...
    >
      ...
      {expanded && (
        ...
        >
          <div
            style={{ display: 'flex', gap: 16, marginBottom: 6, fontSize: 10, color: '#6f8c84' }}
          >
  ...
  return (
    <div className="data-panel">
      ...
      >
        ...
        {critical.length > 0 && (
          <span style={{ color: '#ff2d00', fontSize: 11 }}>{critical.length} CRITICAL</span>
        )}
        {high.length > 0 && (
          <span style={{ color: '#ff6b35', fontSize: 11 }}>{high.length} HIGH</span>
        )}
      </div>
