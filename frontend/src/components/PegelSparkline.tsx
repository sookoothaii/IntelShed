...
export default function PegelSparkline({
  uuid,
  hours = 24,
  width = 220,
  height = 56,
  ...
  if (error) {
    return (
      <div className="sparkline-error" title={error}>
        ⚠ no history
      </div>
    );
  }
  ...
  return (
    <div className={`sparkline ${compact ? 'sparkline-compact' : ''}`}>
      <svg
        ...
        aria-label={`${data.name} ${hours}h history`}
      >
        <defs>
        ...
        <path
          d={`${path} L ${pad + (pts.length - 1) * stepX} ${pad + h} L ${pad} ${pad + h} Z`}
          fill={`url(#spark-grad-${uuid})`}
        />
        <path d={path} fill="none" stroke={color} strokeWidth={1.4} />
        <circle
          cx={pad + (pts.length - 1) * stepX}
          cy={pad + h - ((last.v - lo) / range) * h}
          r={2.4}
          fill={dirColor}
        />
      </svg>
      {!compact && (
        <div className="sparkline-meta">
          <span style={{ color: dirColor }}>
            {dirArrow} {Math.abs(delta).toFixed(1)} {data.unit}
          </span>
          <span style={{ color: '#6f8c84' }}>
            {hours}h · n={pts.length}
          </span>
          <span style={{ color: '#00e5a0' }}>
            now {last.v.toFixed(1)} {data.unit}
          </span>
        </div>
      )}
    </div>
