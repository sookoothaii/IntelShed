...
function applyMapModeToMap(map: MapLibreMap, mode: MapViewMode) {
  ...
  if (map.getLayer(BUILDINGS_LAYER_ID)) {
    map.setLayoutProperty(
      BUILDINGS_LAYER_ID,
      'visibility',
      mode.buildings && mode.render3d ? 'visible' : 'none',
    );
  }
  ...
  return (
    <div className="map-wrap">
      ...
      <div className="map-controls">
            ...
            >
              {archives.map((a) => (
                <option key={a.name} value={a.name}>
                  {a.name} ·{' '}
                  {a.size_mb >= 1024 ? `${(a.size_mb / 1024).toFixed(1)} GB` : `${a.size_mb} MB`}
        ...
        <button className="map-hint-toggle" onClick={() => setShowHint((v) => !v)} type="button">
