...
export default function BriefingKanban({ onClose }: Props) {
  ...
  return (
    <div className="kanban-overlay" role="dialog" aria-label="Briefing Pipeline Kanban">
      ...
      {error && <div className="kanban-empty kanban-empty--error">Failed to load pipeline: {String(error)}</div>}
                        ...
                        {items.map((item, idx) => (
                          <Draggable key={item.item_id} draggableId={item.item_id} index={idx}>
                            {(prov, snap) => (
                              ...
                              >
                                <div className="kanban-card-type" style={{ color: STAGE_COLORS[stage] || 'var(--accent-dim)' }}>
                                  {item.item_type.toUpperCase()}
                                </div>
                                <div className="kanban-card-title">{item.title || item.item_id}</div>
                                {item.confidence > 0 && (
                                  <div className="kanban-card-conf">
                                    <div
                                      className="kanban-card-conf-bar"
                                      style={{ width: `${Math.round(item.confidence * 100)}%`, background: STAGE_COLORS[stage] || 'var(--accent-dim)' }}
                                    />
                                ...
                                {item.sources.length > 0 && (
                                  <div className="kanban-card-sources">
                                    {item.sources.slice(0, 3).map((s) => (
                                      <span key={s} className="kanban-card-source">{s}</span>
                                    ))}
                                    {item.sources.length > 3 && (
                                      <span className="kanban-card-source kanban-card-source--more">+{item.sources.length - 3}</span>
                                    )}
                                  </div>
                                )}
