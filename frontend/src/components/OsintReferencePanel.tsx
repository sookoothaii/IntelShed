  ...
  return (
    <div className="osint-ref-panel">
      <div className="osint-ref-intro">
        <p>
          Operator toolkit catalog — {stats.total} tools. WorldBase keeps live feeds in the globe and briefing;
          everything here opens in a <strong>new tab</strong> (no scraping, no TOS risk).
        </p>
        <ul className="osint-ref-legend">
          <li><span className="osint-rel osint-rel--native">IN STACK</span> — already wired (API / layer)</li>
          <li><span className="osint-rel osint-rel--complement">COMPLEMENT</span> — richer UI for same domain</li>
          <li><span className="osint-rel osint-rel--link">LINK OUT</span> — external only</li>
          <li><span className="osint-rel osint-rel--ref">REFERENCE</span> — manuals / wikis</li>
        </ul>
      </div>
      ...
      <div className="osint-ref-builder">
        ...
        <p className="osint-ref-builder-hint">
          Enter coordinates or identifiers — preview deep-links before opening. Leave blank for home pages only.
        </p>
        <div className="osint-ref-builder-grid">
          <label>LAT<input value={latInput} onChange={(e) => setLatInput(e.target.value)} placeholder="13.7563" /></label>
          <label>LON<input value={lonInput} onChange={(e) => setLonInput(e.target.value)} placeholder="100.5018" /></label>
          <label>ICAO/HEX<input value={icaoInput} onChange={(e) => setIcaoInput(e.target.value)} placeholder="8963f0" /></label>
          <label>MMSI<input value={mmsiInput} onChange={(e) => setMmsiInput(e.target.value)} placeholder="567123456" /></label>
          <label>DOMAIN<input value={domainInput} onChange={(e) => setDomainInput(e.target.value)} placeholder="example.com" /></label>
          <label>USERNAME<input value={userInput} onChange={(e) => setUserInput(e.target.value)} placeholder="handle" /></label>
        </div>
                ...
                return (
                  <article key={tool.id} className="osint-ref-card">
                    <div className="osint-ref-card-top">
                      <span className={`osint-rel osint-rel--${tool.stackRelation === 'link-only' ? 'link' : tool.stackRelation === 'reference' ? 'ref' : tool.stackRelation}`}>
                    ...
                    <div className="osint-ref-tags">
                      {tool.tags.map((tag) => (
                        <span key={tag} className="osint-ref-tag">{tag}</span>
                      ))}
                    </div>
      ...
      <footer className="osint-ref-footer">
        <p>
          Catalog: {OSINT_TOOLS.length} tools across {OSINT_CATEGORIES.length} categories (native + Tier A/B link-out).
        </p>
        <p>
          <strong>S2U Map:</strong> set <code>VITE_S2U_MAP_URL</code> in <code>frontend/.env</code> for your ArcGIS Experience URL.
        </p>
        <p>
          <strong>Backend enrich:</strong> <code>/api/osint/domain</code> adds crt.sh subdomains; <code>/api/osint/email</code> adds HIBP when <code>HIBP_API_KEY</code> is set in <code>backend/.env</code>.
        </p>
        <p>
          <strong>Do not ingest</strong> LiveUAMap, Downdetector, FR24, or Shodan into briefing — link-out keeps licenses clean.
        </p>
      </footer>
    </div>
  )
