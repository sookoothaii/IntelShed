...
function ProvenanceChainInner({ entityId, compact = false }: ProvenanceChainProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchApi(`/api/intel/entity/${encodeURIComponent(entityId)}/provenance`).then((r) => r.ok ? r.json() : null),
      fetchApi(`/api/intel/statements/provenance/summary?entity_id=${encodeURIComponent(entityId)}`).then((r) => r.ok ? r.json() : null),
      fetchApi(`/api/intel/statements/conflicts?entity_id=${encodeURIComponent(entityId)}`).then((r) => r.ok ? r.json() : null),
    ])
      .then(([prov, summ, conf]: [EntityProvenance | null, ProvenanceSummary | null, ConflictsResponse | null]) => {
        if (!active) return
        setProvenance(prov)
        setSummary(summ)
  ...
  return (
    ...
    >
      ...
      <div className="provenance-chain-header">
        ...
        {conflictCount > 0 && (
          <span className="provenance-chain-badge provenance-chain-badge--conflict" title="Cross-dataset value disputes">
            ⚠ {conflictCount} conflict{conflictCount > 1 ? 's' : ''}
          </span>
        )}
        {corroboratedProps > 0 && (
          <span className="provenance-chain-badge provenance-chain-badge--corrob" title="Properties confirmed by multiple sources">
          ...
          {Object.entries(provenance.by_prop)
            ...
            .map(([prop, info]) => (
              <div key={prop} className="provenance-chain-corrob-row">
                ...
                <span className="provenance-chain-corrob-sources">
                  {info.datasets.join(', ')}
                </span>
                <span className="provenance-chain-corrob-count">{info.datasets.length} sources</span>
              </div>
            ))}
        </div>
      )}
    ...
    <div className="provenance-chain" role="region" aria-label="Global provenance statistics">
      ...
      <div className="provenance-chain-rows">
        {datasets.map(([ds, count]) => (
          <div key={ds} className="provenance-chain-row">
            <i className="provenance-chain-dot" style={{ background: 'var(--green)' }} aria-hidden="true" />
            <span className="provenance-chain-dataset">{ds}</span>
            <span className="provenance-chain-count">{count}</span>
          </div>
        ))}
