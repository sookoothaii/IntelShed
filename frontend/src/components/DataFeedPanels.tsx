      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(data?.articles || []).slice(0, 40).map((a: GdeltArticle, i: number) => (
          <div key={i} className="iss-card">
            ...
            {a.url && (
              <a className="tp-link" href={a.url} target="_blank" rel="noreferrer">OPEN ARTICLE ↗</a>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(data?.items || []).map((item: OutageItem, i: number) => (
          <div
            key={i}
            className="iss-card"
            style={{ cursor: item.lat != null ? 'pointer' : 'default', borderLeft: '3px solid #ff6b35' }}
            onClick={() => item.lat != null && item.lon != null && onFocus({
              kind: 'outage',
              lon: item.lon,
              lat: item.lat,
              height: 800000,
              title: item.title || 'Internet outage',
              lines: [`Source: ${item.source || 'ioda'}`, `Status: ${item.status || '—'}`, `Type: ${item.type || '—'}`],
            })}
          >
            <span style={{ color: '#ff6b35', fontWeight: 'bold' }}>{(item.source || 'IODA').toUpperCase()}</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {(data?.alerts || []).slice(0, 50).map((a: HazardsAlert, i: number) => (
          <div
            key={i}
            className="iss-card"
            style={{ cursor: a.lat != null ? 'pointer' : 'default', borderLeft: '3px solid #ffd23f' }}
            onClick={() => a.lat != null && a.lon != null && onFocus({
              kind: 'hazard',
              lon: a.lon,
              lat: a.lat,
              height: 400000,
              title: a.title || a.event || 'Weather alert',
              lines: [`Source: ${a.source || '—'}`, `Severity: ${a.severity || '—'}`, a.area || ''],
            })}
          >
            <span style={{ color: '#ffd23f', fontWeight: 'bold' }}>{(a.source || 'CAP').toUpperCase()}</span>
...
export function VolcanoesPanel({ onFocus }: { onFocus: FocusFn }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const r = await fetchApi(`/api/volcanoes?active_only=${activeOnly ? 'true' : 'false'}&limit=200`)
      if (!r.ok) throw new Error(`${r.status}`)
      setData(await r.json())
    } catch (e) {
      setError((e as Error).message)
    ...
    >
      <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
        {(['regional', 'global', 'all'] as const).map((s) => (
          <button key={s} className={scope === s ? 'on' : ''} onClick={() => setScope(s)} style={{ opacity: scope === s ? 1 : 0.6, marginRight: 6 }}>
            {s.toUpperCase()}
          </button>
        ))}
      </div>
