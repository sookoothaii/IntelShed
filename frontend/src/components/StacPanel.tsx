  ...
  const run = async () => {
    ...
    try {
      const u = new URLSearchParams({
        ...
        collection,
        days: String(days),
        cloud_cover_max: String(cloudMax),
        limit: '24',
      });
    ...
    <div className="stac-panel">
      <div className="stac-toolbar">
        ...
        <label>
          ...
          <input
            ...
            max={100}
            value={cloudMax}
            onChange={(e) => setCloudMax(Math.max(0, Math.min(100, Number(e.target.value) || 25)))}
            style={{ width: 60 }}
          />
        </label>
        <button className="data-refresh" onClick={run}>
          ↻ SEARCH
        </button>
        {catalog && (
          <span className="stac-meta">
            {catalog.titiler_configured ? 'TiTiler ✓' : 'static thumbs only'} · upstream:{' '}
            <code>{catalog.endpoint.replace('https://', '')}</code>
          </span>
        )}
      </div>
      ...
      {loading && (
        <div className="data-info">
          Searching {collection} over {region}…
        </div>
      )}
      ...
      <div className="stac-grid">
        {items.map((it) => (
          <div
            key={it.id}
            className={`stac-card ${selected?.id === it.id ? 'selected' : ''}`}
          ...
          >
            ...
            <div className="stac-meta-line">
              ...
              <span
                className={`stac-cloud ${(it.cloud_cover ?? 0) > 50 ? 'high' : (it.cloud_cover ?? 0) > 20 ? 'mid' : 'low'}`}
              >
      ...
      {selected && (
        <div className="stac-detail">
          ...
          <div className="stac-detail-rows">
            <div>
              Collection: <code>{selected.collection}</code>
            </div>
            <div>Datetime: {fmt(selected.datetime)}</div>
            <div>
              Cloud cover:{' '}
              {selected.cloud_cover != null ? selected.cloud_cover.toFixed(2) + '%' : '—'}
            </div>
            <div>BBox: {selected.bbox?.map((n) => n.toFixed(3)).join(', ')}</div>
            {selected.titiler_tiles &&
              Object.entries(selected.titiler_tiles).map(([k, url]) => (
                <div key={k}>
                  TiTiler {k}: <code title={url}>{url.slice(0, 60)}…</code>
                </div>
              ))}
          </div>
        </div>
