  ...
  if (mode.basemap === 'streets') {
    if (useIon) {
      try {
        ...
        layers.addImageryProvider(
          esriImagery(ESRI_STREET_TILES, 'Esri, OpenStreetMap contributors'),
        );
      }
    } else {
    ...
    if (useIon) {
      try {
        ...
        layers.addImageryProvider(
          esriImagery(ESRI_SATELLITE_TILES, 'Esri, Maxar, Earthstar Geographics'),
        );
      }
    } else {
      layers.addImageryProvider(
...
const TELEMETRY_GROUPS: { id: string; label: string; rows: TelemetryEntry[] }[] = [
  {
    ...
    rows: [
      ...
      },
      {
        ...
        statKey: 'satellites',
        color: '#00e5ff',
        tip: 'Satellites of the selected constellation.',
      },
      { layer: 'orbits', label: 'ORBITS', color: '#00e5ff', tip: 'Orbit lines (toggle).' },
      {
        ...
        color: '#ff6b35',
        healthKeys: ['military'],
        tip: 'Aircraft flagged as military.',
      },
      {
        layer: 'maritime',
        label: 'MARITIME',
        ...
        color: '#00e5ff',
        healthKeys: ['maritime'],
        tip: 'AIS vessel positions worldwide.',
      },
      {
        layer: 'transit',
        label: 'TRANSIT',
        ...
        color: '#ffd23f',
        healthKeys: ['transit'],
        tip: 'Public transit vehicles (GTFS realtime).',
      },
      {
        layer: 'trafficCams',
        label: 'TRAFFIC CAMS',
        statKey: 'trafficCams',
        color: '#ff9f1c',
        healthKeys: ['traffic_cams:regional'],
        tip: 'ASEAN traffic cameras (Singapore data.gov.sg).',
      },
    ],
  },
  {
    ...
    rows: [
      {
        ...
        color: '#ff2d00',
        healthKeys: ['quakes'],
        tip: 'Earthquakes ≥ M2.5, last 24 h (USGS).',
      },
      {
        layer: 'events',
        label: 'EVENTS',
        statKey: 'events',
        color: '#ff6b35',
        healthKeys: ['events', 'world'],
        tip: 'Recent entries from event/RSS feeds.',
      },
      {
        layer: 'gdacs',
        label: 'GDACS',
        ...
        color: '#ff6b35',
        hudKey: 'gdacs',
        healthKeys: ['gdacs', 'gdacs_v2'],
        tip: 'UN alerts: cyclone, earthquake, flood, drought.',
      },
      {
        layer: 'hazards',
        label: 'HAZARDS',
        ...
        color: '#22d3ee',
        hudKey: 'hazards',
        healthKeys: ['hazards', 'cap'],
        tip: 'Official weather warnings (CAP).',
      },
      {
        layer: 'volcanoes',
        label: 'VOLCANOES',
        statKey: 'volcanoes',
        color: '#ff4d5e',
        healthKeys: ['volcanoes'],
        tip: 'Holocene volcanoes (Smithsonian).',
      },
      {
        layer: 'geopolitics',
        label: 'CRISES',
        statKey: 'geopolitics',
        color: '#ff2d00',
        healthKeys: ['geopolitics'],
        tip: 'Geopolitische Krisen (ReliefWeb).',
      },
    ],
  },
  {
    ...
    rows: [
      {
        ...
        color: '#ff6b35',
        hudKey: 'wildfires',
        healthKeys: ['wildfires', 'eonet'],
        tip: 'Active fires (NASA EONET/FIRMS).',
      },
      {
        layer: 'lightning',
        label: 'LIGHTNING',
        ...
        color: '#22d3ee',
        hudKey: 'lightning',
        healthKeys: ['lightning', 'blitzortung'],
        tip: 'Lightning strikes in realtime.',
      },
      {
        layer: 'spaceweather',
        label: 'KP INDEX',
      ...
      },
      {
        ...
        statKey: 'airquality',
        color: '#b0c4b1',
        healthKeys: ['airquality'],
        tip: 'Air quality (AQI/PM2.5) per city.',
      },
      {
        layer: 'weather',
        label: 'WEATHER',
        statKey: 'weather',
        color: '#4fc3f7',
        healthKeys: ['weather', 'windy'],
        tip: 'Surface temp grid (Windy Point Forecast).',
      },
      {
        layer: 'weatherForecast',
        label: 'WX FCST',
        statKey: 'weatherForecast',
        color: '#00e5a0',
        tip: 'Open-Meteo 7-day forecast (severe weather alerts).',
      },
    ],
  },
  {
    ...
    rows: [
      ...
      },
      {
        ...
        statKey: 'piAis',
        color: '#00e5ff',
        tip: 'Pi edge AIS receiver coverage area.',
      },
      {
        layer: 'outages',
        label: 'OUTAGES',
        ...
        hudKey: 'outages',
        healthKeys: ['outages'],
        tip: 'Internet disruptions (IODA/Cloudflare).',
      },
      {
        layer: 'pegel',
        label: 'PEGEL',
        ...
        hudKey: 'pegel',
        healthKeys: ['pegel'],
        tip: 'German river gauges / flood levels.',
      },
      {
        layer: 'energy',
        label: 'ENERGY',
        ...
        color: '#ffd23f',
        hudKey: 'energy',
        healthKeys: ['energy_de'],
        tip: 'German power mix (SMARD).',
      },
      {
        layer: 'osm',
        label: 'OSM POI',
        statKey: 'osm',
        color: '#4fc3f7',
        tip: 'Critical infrastructure POIs (hospitals, power, airports via Overpass).',
      },
    ],
  },
  {
    ...
    rows: [
      {
        ...
        color: '#b794f6',
        healthKeys: ['intel'],
        tip: 'FtM entities with coordinates (24h window).',
      },
      {
        layer: 'osint',
        label: 'OSINT',
        statKey: 'osint',
        color: '#00ffa3',
        tip: 'Your research pins on the globe.',
      },
      {
        layer: 'acled',
        label: 'ACLED',
...
export default function Globe({
  ...
  const applyTarget = useCallback(
    (t: NonNullable<Target>, ent?: Entity) => {
  ...
  const handleTrafficCamSelect = useCallback(
    (next: TrafficCamRef) => {
      applyTarget({
        kind: 'traffic_cam',
        title: `🚦 ${next.name}`,
  ...
  const [ctxMenu, setCtxMenu] = useState<{
    x: number;
    y: number;
    actions: ReturnType<typeof buildContextMenuActions>;
  } | null>(null);
  ...
  const [trailsEnabled, setTrailsEnabled] = useState(
    savedGlobe?.trailsEnabled ?? globeDefaults.trails,
  );
  ...
  useEffect(() => {
    sanctionedRef.current = sanctionedMmsi;
  }, [sanctionedMmsi]);
  ...
  useEffect(() => {
    onOpenWindyRef.current = onOpenWindy;
  }, [onOpenWindy]);
  ...
  useEffect(() => {
    onAddOsintPinRef.current = onAddOsintPin;
  }, [onAddOsintPin]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      (scene.globe as unknown as { atmosphereLightIntensity?: number }).atmosphereLightIntensity =
        12.0;
      ...
      resizeObserver = new ResizeObserver(() => {
        ...
        if (
          ...
          !v ||
          (v as unknown as { isDestroyed?: () => boolean }).isDestroyed?.()
        )
          return;
      ...
      if (GLOBE_IDLE_FPS > 0) {
        ...
        detachIdleWake = () => {
          try {
            ...
            /* canvas/globe already torn down */
          }
        };
      } else {
        detachIdleWake = () => {
      ...
      const pump = () => {
        ...
        const descLimit = quiescent
          ? GLOBE_LOADING_DESCENDANT_QUIESCENT
          : GLOBE_LOADING_DESCENDANT_ACTIVE;
      ...
      const selectEntity = (ent: Entity) => {
        const rawProps = ent.properties as unknown as
          | Record<string, { getValue?: () => unknown } | undefined>
          | undefined;
        ...
        if (kind === 'aircraft') {
          ...
          pick({
            ...
            lines: [
              ...
              ...(sq
                ? [`SQUAWK: ${sq}${['7500', '7600', '7700'].includes(sq) ? ' ⚠ EMERGENCY' : ''}`]
                : []),
            ],
          });
          viewer!.flyTo(ent, { duration: 1.5 });
          ...
          pick({
            kind,
            title: `🚢 ${flagged ? '⚠ ' : ''}${prop('name') || 'Vessel'}`,
            lines: [
              `MMSI: ${mmsi || '—'}`,
              `TYPE: ${prop('type') ?? '—'}`,
              `COURSE: ${prop('course') ?? '—'}°`,
      ...
      handler.setInputAction((click: CesiumMovement) => {
        ...
        if (gp?.entity) {
          ...
          const title = String(
            propRaw('title') || propRaw('name') || propRaw('caption') || kind || 'Entity',
          );
      ...
      const applyVision = (mode: VisionMode) => {
        if (!viewer) return;
        if (activeStage) {
          scene.postProcessStages.remove(activeStage);
          activeStage = null;
        }
        ...
        if (mode === 'nvg') frag = NVG_FRAGMENT;
          ...
          frag = CRT_FRAGMENT;
          uniforms = { aberration: 1.0 };
        } else if (mode === 'night') frag = NIGHT_FRAGMENT;
        if (frag) {
          activeStage = scene.postProcessStages.add(
            new PostProcessStage({ fragmentShader: frag, uniforms }),
          ) as PostProcessStage;
        }
      };
      ...
      apiRef.current = {
        ...
        focusOn: (f: FocusTarget) => {
          ...
          viewer.trackedEntity = undefined;
          const height =
            f.height ?? (f.kind === 'webcam' ? 12000 : f.kind === 'traffic_cam' ? 14000 : 400000);
          viewer.camera.flyTo({
      ...
      timers.push(
        window.setTimeout(() => {
          ...
          fetchApi('/api/gibs/latest')
            .then((r) => r.json())
            .then((d) => {
              gibsDateRef.current = d.date || '';
            })
            .catch(() => {});
  ...
  useEffect(() => {
    apiRef.current.applyVision?.(vision);
  }, [vision]);
  useEffect(() => {
    apiRef.current.setSatGroup?.(satGroup);
  }, [satGroup]);
  useEffect(() => {
    // Hooks handle layer visibility and fetching.
  }, [layers, visible]);
  useEffect(() => {
    apiRef.current.setHeatmap?.(heatmapOn);
  }, [heatmapOn]);
  useEffect(() => {
    apiRef.current.applyMapMode?.();
  }, [mapMode]);
  ...
  const toggleMvt = async () => {
    ...
    if (mvtProvider) {
      ...
      try {
        const mvt = await (
          MVTDataProvider.fromUrl as unknown as (
            url: string,
            options?: Record<string, unknown>,
          ) => Promise<MVTDataProvider>
        )(`http://127.0.0.1:8088/thailand/{z}/{x}/{y}.mvt`, {
          minZoom: 0,
          maxZoom: 16,
        });
        const layer = viewer.imageryLayers.addImageryProvider(
          mvt as unknown as Parameters<Viewer['imageryLayers']['addImageryProvider']>[0],
        );
  ...
  const renderTelemetryRow = (entry: TelemetryEntry) => {
    ...
    const count =
      entry.statKey != null
        ? entry.formatValue
          ? entry.formatValue(stats)
          : String(stats[entry.statKey])
        : on
          ? 'ON'
          : 'OFF';
    ...
    const baseTip = entry.layer === 'spaceweather' ? kpTooltip(stats.spaceweather) : entry.tip;
    const tip = (() => {
    ...
    return (
      <button
        ...
        className={[
          'hud-row',
          'telemetry-row',
          on ? 'telemetry-row--on' : 'telemetry-row--off',
        ].join(' ')}
      ...
      >
        ...
        {health?.status && (
          <span
            className={['telemetry-status', feedStatusClass(health.status)].join(' ')}
            aria-hidden
          />
        )}
    ...
    <div className={`globe-wrap vision-${vision}${layoutSplit ? ' globe-wrap--split' : ''}`}>
          ...
          <div className="telemetry-presets">
            {TELEMETRY_QUICK_PRESETS.map((id) => (
              <button
                ...
                className={[
                  'telemetry-preset',
                  viewPreset === id ? 'telemetry-preset--on' : '',
                ].join(' ')}
      ...
      <div className="globe-hud">
        ...
        <div className="hud-row">
          RENDER<span className="hud-val">{stats.fps} FPS</span>
        </div>
        ...
        <div className="ctl-block">
          ...
          <div className="vision-bar">
            {TRANSIT_CITIES.map((c) => (
              <button
                key={c.id}
                className={transitCity === c.id ? 'on' : ''}
                onClick={() => setTransitCity(c.id)}
              >
                {c.label}
              </button>
            ))}
        ...
        <div className="ctl-block">
          ...
          <div className="vision-bar">
            {(
              ...
              <button
                key={g.id}
                className={gibsLayer === g.id ? 'on' : ''}
                onClick={() => setGibsLayer(g.id)}
              >
                {g.label}
              </button>
            ))}
        ...
        <div className="ctl-block">
          ...
          {layersPanelOpen && (
            <>
              ...
              <label
                className={trailsEnabled ? 'on' : ''}
                style={{ color: '#ffd23f', marginTop: 4 }}
              >
                <input
                  type="checkbox"
                  checked={trailsEnabled}
                  onChange={() => setTrailsEnabled((v) => !v)}
                />
                AIRCRAFT TRAILS
              </label>
              <button
              ...
              {sanctionedMmsi.size > 0 && (
                <div style={{ marginTop: 6, fontSize: 10, color: '#ff2d00' }}>
                  ⚠ {sanctionedMmsi.size} vessel{sanctionedMmsi.size > 1 ? 's' : ''} flagged
                  (OpenSanctions)
                </div>
              )}
              <label className={mvtProvider ? 'on' : ''} style={{ color: '#00e5a0', marginTop: 4 }}>
                <input type="checkbox" checked={!!mvtProvider} onChange={toggleMvt} />
                MVT (EXPERIMENTAL)
              </label>
      ...
      {heatmapOn && heatmapMeta && (
        <div className="fusion-legend">
          ...
          <div className="fusion-legend-contrib">
            {Object.entries(heatmapMeta.contrib)
              .filter(([, n]) => n > 0)
              .map(([k, n]) => (
                <span key={k}>
                  {k}:{n}
