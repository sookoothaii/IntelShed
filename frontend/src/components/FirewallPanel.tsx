...
function RiskBar({ risk }: { risk: number }) {
  const color =
    risk >= 0.9 ? '#ff2d00' : risk >= 0.7 ? '#ff6b35' : risk >= 0.4 ? '#ffaa00' : '#00c853';
  const label = risk >= 0.9 ? 'CRITICAL' : risk >= 0.7 ? 'HIGH' : risk >= 0.4 ? 'MEDIUM' : 'LOW';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 140 }}>
      <div
        style={{ flex: 1, height: 4, background: '#1a2a24', borderRadius: 2, overflow: 'hidden' }}
      >
        <div
          style={{
      ...
      <span style={{ fontSize: 9, color, fontWeight: 'bold', minWidth: 50, textAlign: 'right' }}>
        {label}
      </span>
    </div>
  );
...
function normalizeHistoryEntry(item: Record<string, unknown>): FirewallEntry {
  ...
  return {
    ...
    blocked,
    risk_score: typeof item.risk_score === 'number' ? item.risk_score : blocked && isSlim ? 1 : 0,
...
export default function FirewallPanel() {
  ...
  const stats = {
    ...
    avgLatency:
      history.length > 0
        ? history.reduce((a, h) => a + (h.routing_metadata?.request_queue?.processing_ms || 0), 0) /
          history.length
        : 0,
  };
  ...
  return (
    ...
    >
      ...
      >
        <span>🛡️</span>
        FIREWALL MONITOR
        {slimGuardOn && <StatusBadge status="online" label="SLIM ON" />}
        <StatusBadge
        ...
        />
        {statusDetails && (
          <span style={{ fontSize: 10, color: '#6f8c84', marginLeft: 'auto' }}>
            {statusDetails}
          </span>
        )}
      </h2>
      ...
      >
        ...
        <StatBox
          ...
          color={stats.avgRisk >= 0.5 ? '#ff6b35' : '#00c853'}
        />
                ...
                {filtered.map((entry, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                      <LayerBadge
                        layer={
                          entry.score_origin?.layer || entry.source || entry.evidence_type || '—'
                        }
                      />
                    </td>
                    <td style={{ padding: '6px 10px', textAlign: 'center' }}>
...
function StatBox({
  ...
  return (
    <div style={{ textAlign: 'center', padding: '6px 4px' }}>
      <div style={{ fontSize: 18, fontWeight: 'bold', color, fontFamily: 'monospace' }}>
        {value}
      </div>
      <div
        style={{
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          style={{ marginLeft: 'auto', fontSize: 10, color: '#6f8c84', fontFamily: 'monospace' }}
        >
      ...
      <div style={{ fontSize: 10, color: '#8a9a94', lineHeight: 1.5 }}>
        <div>
          <strong style={{ color: '#c8d4cf' }}>Evidence:</strong> {result.evidence_type || '—'}
        </div>
        {result.score_origin?.layer && (
          <div>
            <strong style={{ color: '#c8d4cf' }}>Layer:</strong> {result.score_origin.layer}
          </div>
        )}
        {result.score_origin?.primary_cause && (
          <div>
            <strong style={{ color: '#c8d4cf' }}>Cause:</strong> {result.score_origin.primary_cause}
          </div>
        )}
        {result.tags && result.tags.length > 0 && (
          <div>
            <strong style={{ color: '#c8d4cf' }}>Tags:</strong> {result.tags.join(', ')}
          </div>
        )}
      </div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: '#6f8c84' }}>
      ...
      {entry.cognitive_probes && (
        <DetailSection title="Cognitive Probes">
          ...
          <DetailRow
            label="Risk"
            value={
              entry.cognitive_probes.risk_score != null
                ? `${(entry.cognitive_probes.risk_score * 100).toFixed(1)}%`
                : '—'
            }
          />
          <DetailRow
            label="Confidence"
            value={
              entry.cognitive_probes.confidence != null
                ? `${(entry.cognitive_probes.confidence * 100).toFixed(1)}%`
                : '—'
            }
          />
          <DetailRow
            label="Tier"
            value={entry.cognitive_probes.tier_used || entry.cognitive_probes.tier || '—'}
          />
          <DetailRow
            label="Latency"
            value={
              entry.cognitive_probes.latency_ms != null
                ? `${Math.round(entry.cognitive_probes.latency_ms)}ms`
                : '—'
            }
          />
        </DetailSection>
      ...
      {entry.semantic_intelligence && (
        <DetailSection title="Semantic Intelligence">
          <DetailRow
            ...
            value={entry.semantic_intelligence.primary_intent || '—'}
          />
          <DetailRow
            label="Intent Confidence"
            value={
              entry.semantic_intelligence.intent_confidence != null
                ? `${(entry.semantic_intelligence.intent_confidence * 100).toFixed(1)}%`
                : '—'
            }
          />
          <DetailRow
            label="Router Decision"
            value={entry.semantic_intelligence.router_decision || '—'}
          />
          {entry.semantic_intelligence.all_scores && (
            <div style={{ marginTop: 6 }}>
              {Object.entries(entry.semantic_intelligence.all_scores).map(
                ([k, v]: [string, { vector_score: number }]) => (
                  <div
                    key={k}
                    style={{
                      display: 'flex',
      ...
      {entry.routing_metadata && (
        <DetailSection title="Routing Metadata">
          {entry.routing_metadata.perimeter_fast_path != null && (
            <DetailRow
              label="Perimeter Fast Path"
              value={entry.routing_metadata.perimeter_fast_path ? 'YES' : 'NO'}
            />
          )}
          {entry.routing_metadata.perimeter_processing_time_ms != null && (
            <DetailRow
              label="Perimeter Latency"
              value={`${Math.round(entry.routing_metadata.perimeter_processing_time_ms)}ms`}
            />
          )}
          {entry.routing_metadata.zedd_latency_ms != null && (
            <DetailRow
              label="ZEDD Latency"
              value={`${Math.round(entry.routing_metadata.zedd_latency_ms)}ms`}
            />
          )}
          {entry.routing_metadata.request_queue?.processing_ms != null && (
            <DetailRow
              label="Total Processing"
              value={`${Math.round(entry.routing_metadata.request_queue.processing_ms)}ms`}
            />
          )}
...
function DetailRow({ label, value }: { label: string; value: string | number }) {
  return (
    <div
      style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 10 }}
    >
